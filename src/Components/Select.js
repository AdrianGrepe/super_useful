...
const Select = ({onSelected=s=>s, defaultValue, style={},selectedOption,disabled = false,options, ...props}) => {
    ...
    return (
        <>
            <div  className={classes.dropDownContainer} {...props}>
                <div style={style} className={classes.dropDownHeader} onClick={toggling}>
                    ...
                    <svg style={{}} width="16" height="8" viewBox="0 0 16 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        {options.map(option => (
                            <li style={{backgroundColor: option === selectedOption? 'rgba(182, 106, 0, .25)' : null, color:disabled?'#A5A5A5':'', cursor:disabled?'not-allowed':'pointer'}} className={`${classes.listItem}`} onClick={disabled?null: onOptionClicked(option)} key={Math.random()}>
                                <span style={{padding:'0', fontSize:'16px', width:'190px', }} className='tooltipLeft'>
                                    <span style={{fontFamily:"'Roboto', sans-serif"}}>{option.attributes.brandName}</span>
                                </span>
                            </li>   
                        ))}
                        </ul>
